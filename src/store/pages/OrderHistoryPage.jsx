...
function OrderHistoryPage() {
  ...
  const getReturnStatusInfo = (status) => {
    switch (status) {
      case "PENDING": return { text: "Pending", color: "text-yellow-600", bg: "bg-yellow-100" };
      case "APPROVED": return { text: "Approved", color: "text-blue-600", bg: "bg-blue-100" };
      case "REJECTED": return { text: "Rejected", color: "text-red-600", bg: "bg-red-100" };
      case "COMPLETED": return { text: "Completed", color: "text-green-600", bg: "bg-green-100" };
      default: return { text: status, color: "text-stone-600", bg: "bg-stone-100" };
    }
  };
  ...
  const handlePayRemainingCOD = async (order) => {
    try {
      ...
      showToast("Remaining balance will be collected on delivery (COD).", "success");
  ...
  const getStatusInfo = (status) => {
    switch (status) {
      case "SHIPPING": return { text: "Shipping", code: 2, color: "text-blue-500" };
      case "PROCESSING": return { text: "Processing", code: 1.5, color: "text-blue-500" };
      case "COMPLETED": return { text: "Delivered", code: 3, color: "text-green-600" };
      case "CANCELLED": return { text: "Cancelled", code: 4, color: "text-red-500" };
      default: return { text: "Pending", code: 1, color: "text-stone-500" };
    }
  };

  const filteredOrders = orders.filter(o => activeTab === "All" || o.status === activeTab);
  const toggleOrderDetail = (id) => setExpandedOrderId(prev => prev === id ? null : id);
  ...
  return (
    <div className="min-h-screen bg-stone-50 pt-24 pb-20 px-6 font-sans">
      ...
      <div className="max-w-4xl mx-auto">
        <h1 className="text-2xl font-bold text-stone-900 mb-6 tracking-tight uppercase">Your Orders</h1>
        ...
        <div className="flex overflow-x-auto hide-scrollbar bg-white rounded-2xl shadow-sm border border-stone-100 mb-8 p-1">
          {TABS.map((tab) => (
            <button
              ...
              className={`flex-1 min-w-[120px] py-3 text-sm font-medium rounded-xl transition-all duration-300 ${activeTab === tab.id ? "bg-blue-600 text-white shadow-md" : "text-stone-500 hover:text-stone-900 hover:bg-stone-50"
                }`}
            >
              {tab.label}
            </button>
                ...
                >
                  <div className="flex justify-between items-start border-b border-stone-100 pb-4 mb-5">
                    <div>
                      ...
                      <p className="text-xs md:text-sm text-stone-500 mt-1">Ordered on {order.date}</p>
                    </div>
                    <div className="text-right">
                      <p className={`font-semibold text-sm ${statusInfo.color} flex items-center justify-end gap-1.5`}>{statusInfo.text}</p>
                      <div className="flex flex-col items-end mt-1">
                        <p className="font-bold text-stone-900 text-lg">{order.total?.toLocaleString("en-US")}₫</p>
                        ...
                        {order.depositType === "FULL" && (
                          <div className="flex flex-col items-end mt-1">
                            <span className="text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md text-[11px] font-bold border border-emerald-100">Full Prepayment ({order.paymentMethod})</span>
                            {order.status === "PENDING" && order.items?.some(i => i.isPreorder) && <span className="text-amber-600 font-medium italic mt-1 text-[10px]">Waiting for stock...</span>}
                          </div>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="flex items-center justify-between gap-4 flex-wrap">
                        <div className="flex items-center gap-4">
                          <div className="w-16 h-16 bg-stone-50 rounded-xl border border-stone-100 flex items-center justify-center overflow-hidden flex-shrink-0">
                            {order.items[0]?.image ? <img src={order.items[0].image} alt={order.items[0].name} className="w-full h-full object-cover" /> : <FiBox size={24} className="text-stone-300" />}
                          </div>
                          <div>
                            <p className="font-semibold text-stone-800 text-sm md:text-base">{order.items[0]?.name}</p>
                            <p className="text-xs md:text-sm text-stone-500 mt-1">{itemCount > 1 ? `and ${itemCount - 1} more product(s)` : `Quantity: ${order.items[0]?.quantity}`}</p>
                          </div>
                        </div>
                        {(itemCount > 1 || order.items[0]?.quantity > 1) && (
                          <button onClick={() => toggleOrderDetail(order.id)} className="px-4 py-2 border border-stone-200 text-stone-700 font-medium rounded-xl hover:bg-stone-50 transition-colors text-sm">
                  ...
                  {isExpanded && (
                    <div className="space-y-4 mb-6">
                      {order.items.map((item) => {
                        ...
                        return (
                          <div key={item.orderItemId} className="border border-stone-200 rounded-2xl p-4 bg-stone-50">
                            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                              <div className="flex items-center gap-4">
                                <div className="w-16 h-16 bg-white rounded-xl border border-stone-100 flex items-center justify-center overflow-hidden flex-shrink-0">
                                  {item.image ? <img src={item.image} alt={item.name} className="w-full h-full object-cover" /> : <FiBox size={24} className="text-stone-300" />}
                                </div>
                                <div>
                                  <p className="font-semibold text-stone-800 text-sm md:text-base">{item.name}</p>
                                  <p className="text-xs md:text-sm text-stone-500 mt-1 flex items-center gap-2">
                                    Quantity: {item.quantity}
                                    {item.isPreorder && <span className="px-2 py-0.5 bg-amber-100 text-amber-700 rounded text-[10px] font-bold border border-amber-200 uppercase">Pre-order</span>}
                                  </p>
                                </div>
                              </div>
                              {statusInfo.code === 3 && (
                                <div className="flex flex-wrap gap-3 md:justify-end">
                                  {!itReq ? (
                                    <Link to={`/return-request?orderItemId=${item.orderItemId}`} className="px-4 py-2 bg-white border border-stone-200 text-stone-700 font-semibold rounded-xl hover:bg-stone-100 transition-colors text-sm">Return/Exchange</Link>
                                  ) : (
                                    <button disabled className="px-4 py-2 bg-stone-200 border border-stone-200 text-stone-500 font-semibold rounded-xl text-sm cursor-not-allowed">
                                      {itReq.status === "PENDING" && "Submitted"}
                                      {itReq.status === "APPROVED" && "Approved"}
                                      {itReq.status === "REJECTED" && "Rejected"}
                                      {itReq.status === "COMPLETED" && "Completed"}
                                    </button>
                                  )}
                                  <Link to={`/product/${item.productId}#review-form`} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 transition-colors text-sm"><FiCheckCircle />Review</Link>
                                </div>
                              )}
                            </div>
                            ...
                            {itReq && (
                              <div className="mt-4 rounded-xl border border-stone-200 bg-white p-4">
                                <div className="flex items-center justify-between flex-wrap gap-2">
                                  <p className="text-sm font-semibold text-stone-800">Return / Exchange Request</p>
                                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${itStatus.bg} ${itStatus.color}`}>{itStatus.text}</span>
                                </div>
                                {itReq.status === "REJECTED" && (itReq.rejectionReason || itReq.adminNote) && (
                                  <p className="mt-3 text-sm text-red-600 font-medium"><span className="font-semibold">Reason: </span>{itReq.rejectionReason || itReq.adminNote}</p>
                                )}
                              </div>
                            )}
                          </div>
                    ...
                    {order.depositType === "PARTIAL" && order.status !== "PENDING" && order.status !== "CANCELLED" && order.paymentStatus !== "PAID" && (
                      <div className="bg-blue-50/40 border border-blue-100 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        {order.paymentMethod !== "COD" ? (
                          ...
                          <div className="px-3 py-1.5 bg-emerald-50 border border-emerald-100 rounded-xl flex items-center gap-2 text-[10px] font-bold text-emerald-600 shrink-0"><FiCheckCircle size={14} /> BALANCE VIA COD</div>
                        )}
                      </div>
                    )}
                      ...
                      {statusInfo.code === 3 && order.items?.length === 1 && (
                        <>
                          {!returnMap[order.items[0].orderItemId] ? (
                            <Link to={`/return-request?orderItemId=${order.items[0].orderItemId}`} className="px-5 py-2.5 bg-white border border-stone-200 text-stone-700 font-semibold rounded-xl hover:bg-stone-50 text-sm">Return Request</Link>
                          ) : (
                            <button disabled className="px-5 py-2.5 bg-stone-100 border border-stone-200 text-stone-500 rounded-xl text-sm font-semibold">Submitted</button>
                          )}

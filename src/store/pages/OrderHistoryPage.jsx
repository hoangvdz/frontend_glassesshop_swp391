...
function OrderHistoryPage() {
            ...
            try {
                ...
                for (const order of data) {
                    for (const item of order.items || []) {
                        try {
                            const res = await getReturnRequestByOrderItemApi(item.orderItemId);
    ...
    const getReturnStatusInfo = (status) => {
        switch (status) {
            case "PENDING":
                return { text: "Chờ xử lý", color: "text-yellow-600", bg: "bg-yellow-100" };
            case "APPROVED":
                return { text: "Đã duyệt", color: "text-blue-600", bg: "bg-blue-100" };
            case "REJECTED":
                return { text: "Bị từ chối", color: "text-red-600", bg: "bg-red-100" };
            case "COMPLETED":
                return { text: "Đã hoàn tất", color: "text-green-600", bg: "bg-green-100" };
            default:
                                ...
                                >
                                    ...
                                    <div className="flex flex-wrap gap-3 justify-end pt-2">
                                        ...
                                        {statusInfo.code === 3 && (
                                            <>
                                                {(() => {
                                                    const returnInfo = returnMap[firstItem.orderItemId];
